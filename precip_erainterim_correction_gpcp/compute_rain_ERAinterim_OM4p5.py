...
for year in np.arange(fyear, lyear+1):
    dsprecip = xr.open_dataset(f'precip_ERAinterim_{year}_OM4p5_conservative.nc')
    dssnow = xr.open_dataset(f'snow_ERAinterim_{year}_OM4p5_conservative.nc')
    print(dsprecip)
    print(dssnow)
    ...
    dsrain.to_netcdf(f'rain_ERAinterim_{year}_OM4p5_conservative.nc',
